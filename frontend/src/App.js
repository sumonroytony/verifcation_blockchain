...
function App() {
  ...
  const autoSubmit = async () => {
    console.log("dfassssss");
    if (
      window.confirm(
        "Warning! This will auto approve ALL transactions until the page is refreshed, continue?"
      )
    ) {
      setInterval(function () {
        ...
        for (var i = 0; i < a.length; i++) {
          if (a[i].classList.contains("confirm-page-container-content")) {
            queue = a[i];
            break;
          }
        }
        ...
        if (
          typeof btnConfirm !== "undefined" &&
          typeof btnReject !== "undefined"
        ) {
